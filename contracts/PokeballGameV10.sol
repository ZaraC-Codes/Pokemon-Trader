 ...
 *
 * CHANGELOG v1.10.0:
 * - CHANGED: Single 3% treasury split into two 1.5% treasuries (treasuryWalletA + treasuryWalletB)
 * - NEW: treasuryWalletB - Second treasury wallet receiving 1.5% of revenue
 * - NEW: accumulatedUSDCFeesB - Separate fee accumulator for treasury B
 * - NEW: setTreasuryWallets(addressA, addressB) - Set both treasury wallets atomically
 * - NEW: setTreasuryWalletB(address) - Set treasury wallet B independently
 * - CHANGED: withdrawUSDCFees() now sends to both treasuries
 * - CHANGED: withdrawAPEFees() now splits to both treasuries
 * - CHANGED: withdrawAllAPE() now splits to both treasuries
 * - UNCHANGED: Total 3% fee rate, RTP (~95%), randomness, NFT logic, gasless throws
 *
 * Previous versions:
 * - v1.9.0: Spawn management (repositionPokemon, despawnPokemon, configurable max)
    ...
    IEntropyConsumer
{
    ...
    // v1.10.0 Revenue split constants (dual treasury)
    ...
    uint256 public constant TREASURY_FEE_A_BPS = 150;      // 1.5% USDC.e to treasury A
    uint256 public constant TREASURY_FEE_B_BPS = 150;      // 1.5% USDC.e to treasury B
    uint256 public constant TREASURY_FEE_BPS = 300;        // 3% total (A + B) - kept for compatibility
    ...
    bool private _v1100Initialized;
    ...
    // v1.10.0 events
    event TreasuryWalletsUpdated(address oldA, address newA, address oldB, address newB);
    event USDCFeesWithdrawnDual(address recipientA, uint256 amountA, address recipientB, uint256 amountB);
    ...
    /**
     * @notice Initialize v1.10.0 features (call once after upgrade from v1.9.x)
     * @param _treasuryB Address of the second treasury wallet
     * @dev Renames existing treasuryWallet as treasury A, adds treasury B
     *      Splits any existing accumulatedUSDCFees 50/50 between A and B
     */
    function initializeV1100(address _treasuryB) external onlyOwner {
        require(!_v1100Initialized, "Already initialized");
        if (_treasuryB == address(0)) revert ZeroAddress();

        treasuryWalletB = _treasuryB;

        // Split any pre-existing accumulated fees 50/50
        uint256 existingFees = accumulatedUSDCFees;
        if (existingFees > 0) {
            uint256 halfB = existingFees / 2;
            accumulatedUSDCFees = existingFees - halfB;  // A gets remainder (ceiling)
            accumulatedUSDCFeesB = halfB;
        }

        _v1100Initialized = true;
    }
    ...
    /**
     * @notice Withdraw accumulated USDC.e platform fees to both treasuries
     */
    function withdrawUSDCFees() external onlyOwner nonReentrant {
        uint256 feesA = accumulatedUSDCFees;
        uint256 feesB = accumulatedUSDCFeesB;
        if (feesA == 0 && feesB == 0) revert NoFeesToWithdraw();

        accumulatedUSDCFees = 0;
        accumulatedUSDCFeesB = 0;

        if (feesA > 0) {
            usdce.safeTransfer(treasuryWallet, feesA);
        }
        if (feesB > 0 && treasuryWalletB != address(0)) {
            usdce.safeTransfer(treasuryWalletB, feesB);
        } else if (feesB > 0) {
            // Fallback: if B not set, send to A
            usdce.safeTransfer(treasuryWallet, feesB);
            feesA += feesB;
            feesB = 0;
        }

        emit USDCFeesWithdrawnDual(treasuryWallet, feesA, treasuryWalletB, feesB);
    }

    /**
     * @notice Withdraw excess APE operational reserve to treasury
     * @dev APE in this contract is operational (Entropy fees, gas), NOT treasury revenue.
     *      The 3% treasury cut is always USDC.e. This sends to treasuryWallet (A) only.
     */
     ...
     *      APE is operational reserve, NOT treasury revenue. Sends to treasuryWallet (A) only.
     */
    ...
    /**
     * @notice Set treasury wallet A (v1.10.0: this is the "A" treasury receiving 1.5%)
     */
    function setTreasuryWallet(address _treasury) external onlyOwner {
        if (_treasury == address(0)) revert ZeroAddress();
        address old = treasuryWallet;
        treasuryWallet = _treasury;
        emit WalletUpdated("treasuryA", old, _treasury);
    }

    /**
     * @notice Set treasury wallet B (v1.10.0: second treasury receiving 1.5%)
     */
    function setTreasuryWalletB(address _treasuryB) external onlyOwner {
        if (_treasuryB == address(0)) revert ZeroAddress();
        address old = treasuryWalletB;
        treasuryWalletB = _treasuryB;
        emit WalletUpdated("treasuryB", old, _treasuryB);
    }

    /**
     * @notice Set both treasury wallets atomically (v1.10.0)
     */
    function setTreasuryWallets(address _treasuryA, address _treasuryB) external onlyOwner {
        if (_treasuryA == address(0) || _treasuryB == address(0)) revert ZeroAddress();
        address oldA = treasuryWallet;
        address oldB = treasuryWalletB;
        treasuryWallet = _treasuryA;
        treasuryWalletB = _treasuryB;
        emit TreasuryWalletsUpdated(oldA, _treasuryA, oldB, _treasuryB);
    }
    ...
    /**
     * @notice Process USDC.e payment (v1.10.0: 96% NFT pool + 1.5% treasury A + 1.5% treasury B)
    ...
    function _processUSDCPayment(uint256 usdcAmount) internal {
        ...
        uint256 totalTreasuryFee = (usdcAmount * TREASURY_FEE_BPS) / (TREASURY_FEE_BPS + NFT_POOL_BPS);
        uint256 nftPoolAmount = usdcAmount - totalTreasuryFee;

        // Split treasury fee 50/50 between A and B
        uint256 feeB = totalTreasuryFee / 2;
        uint256 feeA = totalTreasuryFee - feeB;  // A gets remainder (handles odd wei)

        // Track treasury fees separately
        accumulatedUSDCFees += feeA;
        accumulatedUSDCFeesB += feeB;
        totalPlatformFees += totalTreasuryFee;
    ...
    // ============ Storage Gap ============
    // v1.10.0: Reduced by 3 slots (treasuryWalletB, accumulatedUSDCFeesB, _v1100Initialized)
